...
# allow overwrite of default ages.lab = 1:n.ages
  ...
  # }
  if(!dir.exists(dir.main)){
    dir.create(dir.main, showWarnings = FALSE, recursive = TRUE)
  ...
  if(out.type == 'pdf'){
    ...
    plot.catch.by.fleet(mod, labels_i=fleet.labels)
    for(i in 1:mod$env$data$n_fleets) { 
      plot.catch.age.comp.bubbles(mod, i=i, labels_i=fleet.labels)
      plot.catch.len.comp.bubbles(mod, i=i, labels_i=fleet.labels)
      plot.catch.caal.bubbles(mod, i=i, labels_i=fleet.labels)
    }
    plot.index.input(mod, labels_i=index.labels)
    for(i in 1:mod$env$data$n_indices) {
      plot.index.age.comp.bubbles(mod, i=i, labels_i=index.labels)
      plot.index.len.comp.bubbles(mod, i=i, labels_i=index.labels)
      plot.index.caal.bubbles(mod, i=i, labels_i=index.labels)
    }
    plot.waa(mod,"ssb")
    plot.waa(mod,"jan1")
    ...
    plot.catch.4.panel(mod, labels_i = fleet.labels)
    plot.index.4.panel(mod, labels_i = index.labels)
    if(!all(mod$env$data$Ecov_model == 0) & mod$is_sdrep) plot.ecov.diagnostic(mod)
    plot.NAA.4.panel(mod)
    plot.catch.age.comp(mod, labels_i = fleet.labels)
    plot.catch.len.comp(mod, labels_i = fleet.labels)
    plot.catch.age.comp.resids(mod, labels_i = fleet.labels)
    plot.catch.len.comp.resids(mod, labels_i = fleet.labels)
    plot.catch.caal.resids(mod, labels_i = fleet.labels)
    plot.index.age.comp(mod, labels_i = index.labels)
    plot.index.len.comp(mod, labels_i = index.labels)
    plot.index.age.comp.resids(mod, labels_i = index.labels)
    plot.index.len.comp.resids(mod, labels_i = index.labels)
    plot.index.caal.resids(mod, labels_i = index.labels)
    plot.NAA.res(mod)
    plot.waa.resids(mod)

    if(!is.null(mod$osa)) {
      plot.catch.age.comp.resids(mod, osa = TRUE, labels_i = fleet.labels)
      plot.catch.len.comp.resids(mod, osa = TRUE, labels_i = fleet.labels)
      plot.catch.caal.resids(mod, osa = TRUE, labels_i = fleet.labels)
      plot.index.age.comp.resids(mod, osa = TRUE, labels_i = index.labels)
      plot.index.len.comp.resids(mod, osa = TRUE, labels_i = index.labels)
      plot.index.caal.resids(mod, osa = TRUE, labels_i = index.labels)
  ...
  if(out.type %in% c('png','html')){
    ...
    dev.off()
    for(i in 1:mod$env$data$n_fleets){
      plot.catch.age.comp.bubbles(mod, do.png = TRUE, fontfam=fontfam, i=i, od=dir.data, labels_i=fleet.labels)
      plot.catch.len.comp.bubbles(mod, do.png = TRUE, fontfam=fontfam, i=i, od=dir.data, labels_i=fleet.labels)
      plot.catch.caal.bubbles(mod, do.png = TRUE, fontfam=fontfam, i=i, od=dir.data, labels_i=fleet.labels)
    }    
    ...
    dev.off()
    for(i in 1:mod$env$data$n_indices){
      plot.index.age.comp.bubbles(mod, do.png = TRUE, fontfam=fontfam, i=i, od=dir.data, labels_i=index.labels)
      plot.index.len.comp.bubbles(mod, do.png = TRUE, fontfam=fontfam, i=i, od=dir.data, labels_i=index.labels)
      plot.index.caal.bubbles(mod, do.png = TRUE, fontfam=fontfam, i=i, od=dir.data, labels_i=index.labels)
    }
    ...
    dev.off()
    for(i in 1:mod$env$data$n_fleets){
      plot.catch.4.panel(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = fleet.labels)
      plot.catch.age.comp(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = fleet.labels)
      plot.catch.len.comp(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = fleet.labels)
      plot.catch.age.comp.resids(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = fleet.labels)
      plot.catch.len.comp.resids(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = fleet.labels)
      plot.catch.caal.resids(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = fleet.labels)
    }
    for(i in 1:mod$env$data$n_indices){
      plot.index.4.panel(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = index.labels)
      plot.index.age.comp(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = index.labels)
      plot.index.len.comp(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = index.labels)
      plot.index.age.comp.resids(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = index.labels)
      plot.index.len.comp.resids(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = index.labels)
      plot.index.caal.resids(mod, do.png = TRUE, fontfam=fontfam, use.i=i, od=dir.diag, labels_i = index.labels)
    }
    ...
    if(!is.null(mod$osa)) {
      plot.catch.age.comp.resids(mod, osa = TRUE, do.png=TRUE, fontfam=fontfam, res=res, od=dir.diag, labels_i = fleet.labels)
      plot.catch.len.comp.resids(mod, osa = TRUE, do.png=TRUE, fontfam=fontfam, res=res, od=dir.diag, labels_i = fleet.labels)
      plot.catch.caal.resids(mod, osa = TRUE, do.png=TRUE, fontfam=fontfam, res=res, od=dir.diag, labels_i = fleet.labels)
      plot.index.age.comp.resids(mod, osa = TRUE, do.png=TRUE, fontfam=fontfam, res=res, od=dir.diag, labels_i = index.labels)
      plot.index.len.comp.resids(mod, osa = TRUE, do.png=TRUE, fontfam=fontfam, res=res, od=dir.diag, labels_i = index.labels)
      plot.index.caal.resids(mod, osa = TRUE, do.png=TRUE, fontfam=fontfam, res=res, od=dir.diag, labels_i = index.labels)
    ...
    dev.off()
      ...
      dev.off()
      for(i in 1:mod$env$data$n_fleets){
        png(file.path(dir.res, paste0("predicted_weight_at_age_fleet",i,".png")),width=10,height=10,units="in",res=res,family=fontfam)
        plot.pred.waa(mod,"fleets", ind=i)
        dev.off()
      }
      for(i in 1:mod$env$data$n_indices){
        png(file.path(dir.res, paste0("predicted_weight_at_age_index",i,".png")),width=10,height=10,units="in",res=res,family=fontfam)
	...
	png(file.path(dir.res,"maturity_model.png"),width=10,height=10,units="in",res=res,family=fontfam)
    plot.maturity(mod, type = 'model')
    dev.off()
    condQ = as.list(mod$sdrep, "Std. Error")$logit_q
