...
mohns_rho = function(model)
{
  ...
  if(npeels)
  {
	...
	if(model$env$data$recruit_model == 3) {
		rho.msy = mean(sapply(1:npeels, function(x) (model$peels[[x]]$rep$SSB[ny-x]/exp(model$peels[[x]]$rep$log_SSB_MSY[ny-x]))/(model$rep$SSB[ny-x]/exp(model$rep$log_SSB_MSY[ny-x])) - 1)) 
		names(rho.msy) = "SSB_MSY"
		rho = c(rho, rho.msy)
	}
