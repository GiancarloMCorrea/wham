set_random <- function(input){
  ...
  if(any(input$data$LAA_re_model > 1)) random = c(random, "LAA_re")
  if(any(input$data$WAA_re_model > 1)) random = c(random, "WAA_re")
  if(any(input$data$mat_re_model > 1)) random = c(random, "mat_re")
